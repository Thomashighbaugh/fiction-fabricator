...
class ContentGenerator:
    ...
    async def generate_plot_outline(self, book_spec: BookSpec) -> PlotOutline | None:
        ...
        try:
            ...
            try:
                ...
                )  # Initialize acts as lists
                acts_text = generated_text.split("Act ")
                if len(acts_text) >= 4:
                    plot_outline.act_one = [
                        p.strip()
                        for p in acts_text[1].split("Act")[0].strip().split("\n")
                        if p.strip()
                    ...
                    plot_outline.act_two = [
                        p.strip()
                        for p in acts_text[2].split("Act")[0].strip().split("\n")
                        if p.strip()
                        and not p.strip().startswith(
                            ("#", "*", "##", "Plot Outline", "Act", ":")
                        )
                    ...
                    plot_outline.act_one = [
                        generated_text
                    ]  # Fallback: assign all to act_one
    ...
    async def generate_chapter_outlines(
        ...
        try:
            ...
            )  # Dynamically determine num_chapters

            variables = {
                "plot_outline": "\n".join(
                    [
                        "Act One:\n" + "\n".join(plot_outline.act_one),
                        "Act Two:\n" + "\n".join(plot_outline.act_two),
                        "Act Three:\n" + "\n".join(plot_outline.act_three),
                    ]
                ),
    ...
    async def generate_scene_outlines(
        ...
        try:
            ...
            # Structure Check - removed for scenes for now, can be added back if needed
            ...
            scene_outlines = []
            try:
                # Regex to find scene headings and summaries, similar to chapters
                ...
                for match in scene_matches:
                    ...
                    if scene_summary:
                        scene_outlines.append(
                            SceneOutline(
                                scene_number=scene_number, summary=scene_summary
                            )
                        )
                        found_scenes += 1
                        logger.debug(
                            f"Parsed scene {chapter_outline.chapter_number}-{scene_number} outline: {scene_summary[:50]}..."
                        )
                if found_scenes < num_scenes:
                    logger.warning(
                        f"Expected {num_scenes} scenes, but only parsed {found_scenes} for chapter {chapter_outline.chapter_number}. "
                        f"Review LLM output for parsing errors."
                    )
                    ...
                    logger.warning(
                        f"LLM generated {found_scenes} scenes, trimming to requested {num_scenes} for chapter {chapter_outline.chapter_number}."
                    )
    ...
    async def enhance_scene_outlines(
        ...
        generated_text = await self.ollama_client.generate_text(
            model_name=self.model_name, prompt=prompt  # Use self.model_name here
        )
        if generated_text:
            scene_outlines = []
            try:
