...
from .content_generation_utils import PlotOutline
...
async def generate_plot_outline(
    ...
    try:
        ...
        generation_prompt = generation_prompt_template(
            **variables
        )  # Corrected: THEN call with variables
        logger.debug("generate_plot_outline - Generation prompt formatted.")
        logger.debug(
            f"generate_plot_outline - Full generation prompt: %s", generation_prompt
        )
        ...
        critique_prompt_template = (
            prompt_manager.create_plot_outline_critique_prompt()
        )  # Correct
        rewrite_prompt_template = (
            prompt_manager.create_plot_outline_rewrite_prompt()
        )  # Correct
        ...
        if critique:
            ...
            generated_text = (
                await ollama_client.generate_text(  # Overwrite with rewritten
            ...
            logger.warning(
                "No critique generated.  Proceeding with original generation."
            )
...
async def enhance_plot_outline(
    ...
    try:
        ...
        current_outline_toml = tomli_w.dumps(current_outline.model_dump_toml())
        variables = {"current_outline_toml": current_outline_toml}
        ...
        """
        validated_text = await content_generator._validate_and_correct_toml(
            enhanced_outline_toml, expected_schema  # Correct: positional
        )
        if not validated_text:
            logger.error("TOML Validation Failed")
            return None
        ...
        try:
            enhanced_outline_dict = tomli.loads(enhanced_outline_toml)
