...
def write_scene_narrative(
    ...
    Returns:
        str: The generated narrative text for the scene. Returns an error message string
             if generation fails.
    """
    scene_title = scene_detailed_outline.get("scene_title", f"Chapter {chapter_number}, Scene {scene_number_in_chapter}")
    logger.Log(f"Initiating narrative generation for Scene: '{scene_title}' (C{chapter_number}.S{scene_number_in_chapter})...", 3)

    if not scene_detailed_outline:
        logger.Log(f"Scene detailed outline for '{scene_title}' is empty or missing. Cannot generate narrative.", 6)
    ...
    try:
        ...
        key_events_str = "\n- ".join(scene_detailed_outline.get("key_events_actions", ["No specific key events listed."]))
        if key_events_str and not key_events_str.startswith("\n- "): key_events_str = "- " + key_events_str # Ensure list format
        
        dialogue_points_str = "\n- ".join(scene_detailed_outline.get("dialogue_points", ["No specific dialogue points listed."]))
        if dialogue_points_str and not dialogue_points_str.startswith("\n- "): dialogue_points_str = "- " + dialogue_points_str

        characters_present_str = ", ".join(scene_detailed_outline.get("characters_present", ["Unknown characters"]))
        
        formatted_prompt = prompt_template.format(
            _OverallStoryOutline=overall_story_outline if overall_story_outline else "Overall story outline not provided.",
            _PreviousSceneContextSummary=previous_scene_or_chapter_context_summary if previous_scene_or_chapter_context_summary else "No immediate prior context available for this scene.",
            _ChapterNumber=chapter_number,
            _SceneNumberInChapter=scene_number_in_chapter,
            _SceneTitle=scene_title,
            _SceneSettingDescription=scene_detailed_outline.get("setting_description", "Setting not specified."),
            ...
            _ScenePacingNote=scene_detailed_outline.get("pacing_note", "Moderate pacing assumed."),
            _SceneTone=scene_detailed_outline.get("tone", "Neutral tone assumed."),
            _ScenePurposeInChapter=scene_detailed_outline.get("purpose_in_chapter", "Purpose not specified."),
            _SceneTransitionOutHook=scene_detailed_outline.get("transition_out_hook", "The scene concludes."),
        ...
        logger.Log(f"Formatting error in OPTIMIZED_SCENE_NARRATIVE_GENERATION prompt for '{scene_title}': Missing key {e}", 7)
        ...
        logger.Log(f"Unexpected error formatting scene narrative prompt for '{scene_title}': {e}", 7)
        return f"Scene Generation Error: Unexpected prompt formatting error for '{scene_title}' - {e}."

    messages: List[Dict[str, Any]] = [
        interface.build_system_query(Prompts.DEFAULT_SYSTEM_PROMPT), # Expert creative writer persona
    ...
    try:
        ...
        if not scene_narrative_text or "Error:" in scene_narrative_text: # Check for errors from safe_generate_text or LLM
            logger.Log(f"LLM failed to generate narrative for scene '{scene_title}' or returned an error.", 6)
        ...
        logger.Log(f"Narrative successfully generated for Scene: '{scene_title}'. Word count: {generated_word_count}", 4)
        
        if generated_word_count < Config.SCENE_NARRATIVE_MIN_WORDS:
             logger.Log(f"Warning: Generated scene '{scene_title}' has {generated_word_count} words, less than minimum {Config.SCENE_NARRATIVE_MIN_WORDS}.", 6)

        return scene_narrative_text

    except Exception as e:
        logger.Log(f"An unexpected critical error occurred during scene narrative generation for '{scene_title}': {e}", 7)
...
if __name__ == "__main__":
    ...
    class MockInterface:
        ...
        def safe_generate_text(self, l, m, mo, min_word_count):
            print(f"Mock LLM Call (safe_generate_text) to {mo} with min_words {min_word_count} for scene narrative.")
            ...
            title_match = re.search(r"Title\": \"(.*?)\"", user_prompt_content)
            mock_title = title_match.group(1) if title_match else "Mocked Scene"

            return [*m, {"role": "assistant", "content": f"This is the mock narrative for scene '{mock_title}'. It describes exciting events and character interactions, fulfilling all requirements with vivid prose. The minimum word count of {min_word_count} is definitely met by this sentence and several more that follow, ensuring a complete and satisfying scene."}]
    ...
    sample_scene_outline = {
        ...
        "key_events_actions": ["Tense dialogue exchange.", "A sudden reveal.", "Hero makes a difficult choice."],
        "dialogue_points": ["Villain: 'You were always a fool.'", "Hero: 'I won't back down.'"],
    ...
    scene_text = write_scene_narrative(
        ...
        prev_context,
        chapter_number=5,
        scene_number_in_chapter=3,
        base_story_context=base_ctx
    )
    
    print(f"\nGenerated Scene Narrative:\n{scene_text}\n")
    assert "The Confrontation" in scene_text # Check if mock used the title
