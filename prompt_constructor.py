...
class PromptConstructor:
    def __init__(self, prompt_engine=None, llm_config=None, summary_config=None):
        if prompt_engine:
           ...
           self.missing_book_spec_messages = prompt_engine['missing_book_spec_messages']
           self.enhance_book_spec_messages = prompt_engine['enhance_book_spec_messages']
           self.create_plot_chapters_messages = prompt_engine['create_plot_chapters_messages']
           self.enhance_plot_chapters_messages = prompt_engine['enhance_plot_chapters_messages']
           self.split_chapters_into_scenes_messages = prompt_engine['split_chapters_into_scenes_messages']
    ...
    @retry(wait=wait_exponential(multiplier=1, min=config.RETRY_WAIT, max=config.RETRY_WAIT * 4),
    ...
    async def make_call(self, prompt, system_message):
        ...
        try:
            response = await self._query_ollama(prompt=prompt, system_message=system_message)
            return response
    ...
    async def _query_ollama(self, prompt, system_message):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                payload = {
                    "model": self.llm_config.model if self.llm_config else config.MODEL_NAME,
                    "messages": [
                ...
                async with session.post(url, json=payload, timeout=config.TIMEOUT) as resp:
                    if resp.status != 200:
                        ...
                        raise AppUsageException(f"Ollama API Error: {resp.status}, {text}")
                    ...
                    if resp.content_type == 'application/x-ndjson':
                        ...
                        async for line in resp.content:
                            if line:
                                try:
                                    ...
                                    all_content += part.get("message", {}).get("content", "")
                        ...
                        if "message" in response_data and "content" in response_data["message"]:
                            ...
                            raise AppUsageException(f"Unexpected response format: {response_data}")
                    else:
                        text = await resp.text()
                        logger.error(f"Unexpected content type: {resp.content_type}")
                        raise AppUsageException(f"Unexpected content type: {resp.content_type}, {text}")
