...
class Project:
    ...
    def setup_new_project(self, idea: str, title: str, synopsis: str) -> None:
    ...
    def save_state(self, filename: str) -> bool:
        ...
        if self.book_root is None or self.book_dir is None:
            self.console.print(
                "[bold red]Error: Cannot save state, project not fully initialized.[/bold red]"
            )
            return False
        ...
        try:
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(xml_str)
            self.console.print(
                f"[green]Project state saved to:[/green] [cyan]{filepath.name}[/cyan]"
            )
            return True
        except Exception as e:
            self.console.print(
                f"[bold red]Error saving project state to {filepath}: {e}[/bold red]"
            )
            return False
    ...
    def apply_patch(self, patch_xml_str: str, is_loading: bool = False) -> bool:
        ...
        try:
            ...
            for chapter_patch in patch_root.findall("chapter"):
                ...
                if not chapter_id:
                    if not is_loading:
                        self.console.print(
                            "[yellow]Warning: Chapter patch missing ID, skipping.[/yellow]"
                        )
                    continue
                ...
                if target_chapter is None:
                    if not is_loading:
                        self.console.print(
                            f"[yellow]Warning: Chapter {chapter_id} not found in book structure, skipping patch.[/yellow]"
                        )
                    continue
            ...
            for element_patch in patch_root:
                ...
                if self.book_root is not None:
                    ...
                    if not is_loading:
                        self.console.print(
                            f"[green]Applied patch for <{element_patch.tag}>.[/green]"
                        )
                    applied_changes = True
        ...
        if not applied_changes and not is_loading:
            self.console.print(
                "[yellow]Patch processed, but no applicable changes were found.[/yellow]"
            )
    ...
    def debug_patch_failure(self, patch_xml_str: str) -> None:
        ...
        try:
            patch_root = utils.parse_xml_string(
                patch_xml_str, self.console, expected_root_tag="patch"
            )
    ...
    def _restore_generated_chapters_set(self) -> None:
        ...
        if chapters_with_content:
            self.console.print(
                f"[green]Restored {len(chapters_with_content)} chapters with existing content.[/green]"
            )
        else:
    ...
    def edit_frontmatter_section(self, section_name: str) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"[yellow]Opening {section_name.replace('_', ' ')} in {editor}...[/yellow]"
            )
            ...
            if result.returncode == 0:
                ...
                if self.book_dir:
                    ...
                    self.console.print(
                        f"[green]✓ {section_name.replace('_', ' ')} updated successfully.[/green]"
                    )
                    return True
            else:
    ...
    def edit_book_title(self) -> bool:
        ...
        new_title = Prompt.ask("[cyan]Enter new book title[/cyan]", default=current_title).strip()
    ...
    def edit_author_name(self) -> bool:
        ...
        new_author = Prompt.ask("[cyan]Enter author name[/cyan]", default=current_author).strip()
