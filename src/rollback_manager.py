...
class RollbackManager:
    ...
    def __init__(self, project_dir: Path, max_attempts: int = 3) -> None:
        """
    ...
    def load_attempts(self) -> None:
        ...
        try:
            with open(self.attempts_file, encoding="utf-8") as f:
                data = json.load(f)

            self.attempts = {}
            for chapter_id, attempts_data in data.items():
                self.attempts[chapter_id] = [GenerationAttempt(**a) for a in attempts_data]
    ...
    def select_best_version(self, chapter_id: str, scoring_function=None) -> str | None:
        """
        ...
        best_attempt = next(
            (a for a in successful_attempts if a.attempt_number == best_attempt_num), None
        )
    ...
    def get_prompt_variations(self, base_prompt: str, attempt_num: int, chapter_id: str) -> str:
        """
...
"""
    ...
    def rollback_to_version(self, chapter_id: str, version_num: int) -> str | None:
        """
        ...
        if (
            target_attempt.status != GenerationStatus.SUCCESS
            or not target_attempt.result_patch_path
        ):
            return None
    ...
    def get_attempts_summary(self, chapter_id: str) -> str:
        ...
        for attempt in attempts:
            ...
            lines.append(f"  {status_emoji} Attempt {attempt.attempt_number}: {status_text}")
            ...
            if attempt.quality_metrics:
                lines.append(
                    f"      Quality Score: {attempt.quality_metrics.get('overall_score', 'N/A'):.1f}"
                )
    ...
    def cleanup_old_versions(self, keep_count: int = 3) -> None:
        """
    ...
    def get_all_chapters_needing_regeneration(self, quality_threshold: float = 6.0) -> list[str]:
        """
    ...
    def regenerate_with_rollback(
        ...
        self.console.print(
            f"[cyan]Starting regeneration for Chapter {chapter_id} (max {max_attempts} attempts)[/cyan]"
        )
            ...
            try:
                ...
                if result_patch:
                    ...
                    if status == GenerationStatus.REJECTED:
                        self.console.print(
                            "[yellow]  Below threshold - will continue to next attempt[/yellow]"
                        )
                    else:
        ...
        best_version = self.select_best_version(
            chapter_id, scoring_function=self._simple_quality_score
        )
    ...
    def _get_current_patch(self, chapter_id: str) -> str | None:
