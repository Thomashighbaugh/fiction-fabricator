...
def display_summary(project):
    ...
    chapters = sorted(
        project.book_root.findall(".//chapter"),
    ...
    console.print(
        Panel(
            f"Book Summary: {project.book_root.findtext('title', 'Untitled')}\nTotal Word Count: {total_wc:,}",
            title=f"Current Status ({project.book_dir.name})",
            border_style="blue",
        )
    )
    ...
    if characters:
        ...
        for char in characters:
            ...
            char_table.add_row(
                utils.get_chapter_id_with_default(char),
                char.findtext("name", ""),
                f"{desc[:100]}...",
            )
    ...
    if chapters:
        ...
        for chap in chapters:
            ...
            summary_status = (
                "[green]✓[/green]" if chap.findtext("summary", "").strip() else "[red]✗[/red]"
            )
...
def prompt_for_new_project_idea(prompt_file: str | None) -> str:
    ...
    while not idea:
        try:
            ...
            console.print(
                "[dim]Enter a brief description of your story idea (a few sentences or paragraphs)[/dim]"
            )
            ...
            if not idea:
                console.print(
                    "[red]Error: Book idea cannot be empty. Please provide a description.[/red]"
                )
        except EOFError:
            # Handle EOF (Ctrl+D or piped empty input)
            console.print("\n[red]Error: No input provided. Please provide a book idea.[/red]")
            console.print(
                "[yellow]Tip: Use --prompt FILE to provide a book idea from a file[/yellow]"
            )
...
def prompt_for_story_type() -> str:
    ...
    choice = bullet_choice("Choose project type:", ["Novel", "Short Story"])
...
def review_and_modify_story_elements(project) -> dict:
    ...
    new_title = Prompt.ask(
        f"[cyan]Title[/cyan] [dim](current: {current_title})[/dim]", default=current_title
    ).strip()

    new_genre = Prompt.ask(
        f"[cyan]Genre[/cyan] [dim](current: {current_genre})[/dim]", default=current_genre
    ).strip()

    new_tone = Prompt.ask(
        f"[cyan]Tone[/cyan] [dim](current: {current_tone})[/dim]", default=current_tone
    ).strip()
...
def edit_character_names(project) -> dict:
    ...
    for char_id, current_name in character_names.items():
        new_name = Prompt.ask(
            f"[cyan]{char_id}[/cyan] [dim](current: {current_name})[/dim]", default=current_name
        ).strip()
        if new_name and new_name != current_name:
            modified_character_names[char_id] = new_name
...
def display_menu(title: str, options: dict) -> str:
    ...
    selected = bullet_choice("Choose an action:", choice_items)
...
def get_chapter_selection(project, prompt_text: str, allow_multiple: bool) -> list[ET.Element]:
    ...
    all_chapter_ids = {
        utils.get_chapter_id(chap) for chap in project.book_root.findall(".//chapter")
    }
    ...
    while True:
        ...
        if invalid_ids:
            console.print(
                f"[red]Invalid chapter IDs: {', '.join(invalid_ids)}. Please try again.[/red]"
            )
            continue

        return [
            project.find_chapter(cid)
            for cid in sorted(selected_ids, key=int)
            if project.find_chapter(cid) is not None
        ]
...
def prompt_for_lorebook_selection() -> str | None:
    ...
    if not Confirm.ask(
        "[yellow]Would you like to use a lorebook for additional world-building context?[/yellow]",
        default=False,
    ):
        return None
    ...
    if not all_lorebooks:
        ...
        console.print(
            "[dim]Create lorebooks in the 'lorebooks/' directory or use --create-lorebook[/dim]"
        )
        return None
    ...
    selected = bullet_choice("Choose a lorebook:", menu_choices)
    ...
    if choice_num == "1":
        ...
        while True:
            import_path = Prompt.ask(
                "[cyan]Enter path to TavernAI/SillyTavern lorebook JSON file[/cyan]", default=""
            ).strip()
            ...
            if not path.suffix.lower() == ".json":
                console.print(
                    f"[yellow]Warning: {import_path} doesn't have a .json extension[/yellow]"
                )
            ...
            output_name = Prompt.ask(
                "[cyan]Enter name for the converted lorebook[/cyan]", default=default_name
            ).strip()
            ...
            try:
                ...
                console.print(
                    f"[green]✓ Successfully imported and converted lorebook to: {output_path}[/green]"
                )
                console.print(
                    f"[green]✓ Converted {len(converted_data['entries'])} entries[/green]"
                )
        ...
        for i, lorebook in enumerate(all_lorebooks, 1):
            # Show just filename for lorebooks/ files, full path for others
            display_name = (
                Path(lorebook).name if str(lorebook).startswith("lorebooks/") else lorebook
            )
            menu_choices.append(f"{i}. {display_name}")
...
def _delete_project(existing_projects: list) -> tuple[bool, str | None, str | None]:
    """
    Helper function to delete a project.

    Args:
        existing_projects: List of Path objects for existing project directories
    ...
    project_choices.append(f"{len(project_choices) + 1}. Cancel")

    selected = bullet_choice("Select a project to delete:", project_choices)
    ...
    if selected_num > len(existing_projects):
        ...
        # Return to main menu
        return prompt_for_project_selection()

    # Get the selected project
    selected_project = existing_projects[selected_num - 1]
    title = "Unknown Title"
    outline_file = selected_project / "outline.xml"
    if outline_file.exists():
        try:
            tree = ET.parse(outline_file)
            root = tree.getroot()
    ...
    console.print("\n[bold red]⚠ WARNING: This will permanently delete the project:[/bold red]")
    console.print(f"[red]Project: {selected_project.name}[/red]")
    console.print(f"[red]Title: {title}[/red]")
    console.print(f"[red]Location: {selected_project.absolute()}[/red]")

    if not Confirm.ask(
        "\n[red]Are you absolutely sure you want to delete this project? This cannot be undone.[/red]",
        default=False,
    ):
        console.print("[dim]Deletion cancelled.[/dim]")
        # Return to main menu
    ...
    # Double confirmation
    if not Confirm.ask(
        "[red]Type 'yes' to confirm deletion. Are you really sure?[/red]", default=False
    ):
        console.print("[dim]Deletion cancelled.[/dim]")
        # Return to main menu
        return prompt_for_project_selection()
    ...
    try:
        shutil.rmtree(selected_project)
        console.print(f"[green]✓ Project '{title}' has been deleted successfully.[/green]")

        # Ask if user wants to delete another project or return to main menu
        if Confirm.ask("\n[cyan]Delete another project?[/cyan]", default=False):
            return _delete_project([p for p in Path("projects").iterdir() if p.is_dir()])
        else:
            # Return to main menu
            return prompt_for_project_selection()
        ...
        console.print("[yellow]Project was not deleted due to an error.[/yellow]")
        # Return to main menu
        return prompt_for_project_selection()
...
def prompt_for_project_selection() -> tuple[bool, str | None, str | None]:
    ...
    if choice_num == "1":
        ...
        selected = bullet_choice("Select a project to load:", project_choices)
    ...
    elif choice_num == "3" and existing_projects:
        # Delete a project
        return _delete_project(existing_projects)

    else:
        # Import character card
        console.print("\n[bold cyan]Import Character Card[/bold cyan]")
        console.print(
            "[dim]Supports TavernAI/SillyTavern character cards (JSON or PNG format)[/dim]"
        )

        file_path = Prompt.ask(
            "[cyan]Enter path to character card file (.json or .png)[/cyan]", default=""
        ).strip()
        ...
        from src.utils import convert_character_card_to_premise, load_character_card
        ...
        if not character_data:
            console.print(
                "[red]Error: Failed to load character card. The file may be corrupted or in an unsupported format.[/red]"
            )
            console.print("[yellow]Starting new project instead.[/yellow]")
            return True, None, None
        ...
        if not Confirm.ask(
            ...
            console.print(
                "[dim]Character card import cancelled, starting new project instead.[/dim]"
            )
            return True, None, None

        if (
            Prompt.ask(
                "\n[yellow]Would you like to edit the premise?[/yellow]",
                choices=["y", "n"],
                default="n",
            )
            == "y"
...
def _edit_chapter(project) -> bool:
    ...
    chapter_num = IntPrompt.ask(
        f"\n[cyan]Enter chapter number to edit (1-{len(chapters)})[/cyan]", default=1
    )
    ...
    new_title = Prompt.ask(
        "[cyan]New title[/cyan] (press Enter to keep current)", default=current_title
    ).strip()

    new_summary = Prompt.ask(
        "[cyan]New summary[/cyan] (press Enter to keep current)", default=current_summary
    ).strip()
...
def _add_chapter(project) -> bool:
    ...
    position = IntPrompt.ask(
        f"[cyan]Insert at position (1-{len(chapters) + 1})[/cyan]", default=len(chapters) + 1
    )
...
def _delete_chapter(project) -> bool:
    ...
    chapter_num = IntPrompt.ask(
        f"\n[cyan]Enter chapter number to delete (1-{len(chapters)})[/cyan]", default=1
    )
...
def _reorder_chapters(project) -> bool:
    ...
    from_pos = IntPrompt.ask(
        f"\n[cyan]Move chapter from position (1-{len(chapters)})[/cyan]", default=1
    )
    ...
    to_pos = IntPrompt.ask(f"[cyan]Move to position (1-{len(chapters)})[/cyan]", default=1)
