...
class LLMClient:
    ...
    def get_response(self, prompt_content: str, task_description: str = "Generating content", allow_stream: bool = False) -> str | None:
        """
        ...
        while retries < config.MAX_API_RETRIES:
            ...
            try:
                self.console.print(
                    Panel(f"[yellow]Sending request to DeepSeek ({task_description})... (Attempt {retries + 1}/{config.MAX_API_RETRIES})[/yellow]", border_style="dim")
                )
                ...
                with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), transient=True) as progress:
                    ...
                    if response.choices[0].message.content:
                        ...
                        self.console.print(f"[cyan]>>> DeepSeek Response ({task_description}):[/cyan]")
                ...
                self.console.print(Panel("[green]✓ DeepSeek response received successfully.[/green]", border_style="dim"))
