  # fiction-fabricator/src/chapters.py
import json
...
def generate_beats(chapter_summary):
    ...
    beats = []
    for beat in beats_text.split("\n"):
        if beat.strip():  # Skip empty lines
            beats.append({"action_point": beat.strip()})
...
def reorder_chapters(chapters_json):
    ...
    while True:
        try:
            ...
            if 0 <= old_index < len(chapter_titles) and 0 <= new_index < len(
                chapter_titles
            ):
...
def merge_chapters(chapters_json):
    ...
    while True:
        try:
            chapter1_index = (
                int(input("Enter the number of the first chapter to merge: ")) - 1
            )
            chapter2_index = (
                int(input("Enter the number of the second chapter to merge: ")) - 1
            )
            if 0 <= chapter1_index < len(chapter_titles) and 0 <= chapter2_index < len(
                chapter_titles
            ):
                ...
                merged_beats = (
                    chapters_json[chapter1_title]["beats"]
                    + chapters_json[chapter2_title]["beats"]
                )
                ...
                chapters_json[new_chapter_title] = {
                    "summary": "",
                    "beats": merged_beats,
                }
...
def customize_beats(chapters_json):
    ...
    while True:
        try:
            chapter_index = (
                int(input("Enter the number of the chapter to customize beats: ")) - 1
            )
            if 0 <= chapter_index < len(chapter_titles):
                chapter_title = chapter_titles[chapter_index]
                beats = chapters_json[chapter_title]["beats"]
                ...
                while True:
                    ...
                    if choice.lower() == "done":
                        ...
                        new_beat = {
                            "action_point": input("Enter the new action point: ")
                        }
                        beats.append(new_beat)
                    elif choice.lower() == "remove":
                        beat_index = (
                            int(input("Enter the number of the beat to remove: ")) - 1
                        )
                        ...
                        beat_index = (
                            int(input("Enter the number of the beat to modify: ")) - 1
                        )
                        if 0 <= beat_index < len(beats):
                            beats[beat_index]["action_point"] = input(
                                "Enter the modified action point: "
                            )
                        else:
                            print("Invalid beat index. Please try again.")
                    else:
                ...
                chapters_json[chapter_title][
                    "beats"
                ] = beats  # Update beats in chapters_json
                break
            else:
