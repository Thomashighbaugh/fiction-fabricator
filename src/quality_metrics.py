...
class QualityEvaluator:
    ...
    def evaluate_chapter(self, content_text: str, chapter_context: dict | None = None) -> QualityMetrics:
        """
        ...
        if word_count < self.thresholds['min_words']:
            issues.append(f"Word count ({word_count}) below minimum ({self.thresholds['min_words']})")
        ...
        if dialogue_expected:
            if dialogue_ratio < self.thresholds['min_dialogue_ratio']:
                issues.append(f"Dialogue ratio ({dialogue_ratio:.2%}) below minimum ({self.thresholds['min_dialogue_ratio']:.2%})")
            elif dialogue_ratio > self.thresholds['max_dialogue_ratio']:
                issues.append(f"Dialogue ratio ({dialogue_ratio:.2%}) above maximum ({self.thresholds['max_dialogue_ratio']:.2%})")
        ...
        if sentence_variety_score < self.thresholds['min_sentence_variety']:
            issues.append(f"Sentence variety score ({sentence_variety_score:.1f}/10) below threshold")
        ...
        if vocabulary_richness_score < self.thresholds['min_vocabulary_richness']:
            issues.append(f"Vocabulary richness score ({vocabulary_richness_score:.1f}/10) below threshold")
    ...
    def _is_dialogue_expected(self, chapter_context: dict) -> bool:
        ...
        non_dialogue_keywords = [
            'internal monologue', 'solitary', 'alone', 'introspection',
            'flashback', 'memory', 'dream', 'nightmare',
            'internal struggle', 'internal conflict', 'contemplation',
            'meditation', 'reflection', 'solitude',
            'descriptive passage', 'scene description', 'setting the scene',
            'travel', 'journey', 'pursuit', 'chase',
            'battle', 'combat', 'fight scene', 'action sequence',
            'exploration', 'discovery', 'observation',
            'planning', 'strategy', 'preparation'
        ]
        
        # Keywords indicating dialogue IS expected
        dialogue_keywords = [
            'conversation', 'dialogue', 'discussion', 'debate',
            'argument', 'confrontation', 'negotiation',
            'meeting', 'gathering', 'council', 'conference',
            'interrogation', 'questioning', 'interview',
            'confession', 'revelation', 'declaration',
            'interact', 'speak with', 'talk to', 'tell',
            'exchange', 'banter', 'quarrel'
        ]
    ...
    def _calculate_overall_score(self, word_count: int, dialogue_ratio: float,
                                sentence_variety: float, vocabulary_richness: float) -> float:
        """
        ...
        if self.thresholds['min_dialogue_ratio'] <= dialogue_ratio <= self.thresholds['max_dialogue_ratio']:
            scores.append(8)
        else:
            distance = min(
    ...
    def generate_quality_report(self, metrics: QualityMetrics, dialogue_expected: bool = True) -> str:
        """
        ...
        if dialogue_expected:
            report.append(f"Dialogue Ratio: {metrics.dialogue_ratio:.1%} (range: {self.thresholds['min_dialogue_ratio']:.1%}-{self.thresholds['max_dialogue_ratio']:.1%})")
        else:
            report.append(f"Dialogue Ratio: {metrics.dialogue_ratio:.1%} (dialogue not required for this chapter type)")
        
        report.append(f"Sentence Variety: {metrics.sentence_variety_score}/10 (min: {self.thresholds['min_sentence_variety']})")
        report.append(f"Vocabulary Richness: {metrics.vocabulary_richness_score}/10 (min: {self.thresholds['min_vocabulary_richness']})")
