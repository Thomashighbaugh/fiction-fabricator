...
class OllamaClient(LLMClientInterface):
    ...
    def _init_client(self) -> ollama.Client | None:
        ...
        try:
            ...
            if not models.models:
                self.console.print("[yellow]No models found in Ollama. Please pull a model first.[/yellow]")
                self.console.print("[yellow]Example: ollama pull llama3.2:3b[/yellow]")
                return None
                
            self.console.print(f"[green]Successfully connected to Ollama[/green]")
            self.console.print(f"[dim]Available models: {', '.join([m.name for m in models.models])}[/dim]")
            return client
    ...
    def get_response(self, prompt: str, task_description: str = "Generating content", allow_stream: bool = True) -> Optional[str]:
        ...
        while retries < config.MAX_API_RETRIES:
            ...
            try:
                self.console.print(
                    Panel(f"[yellow]Sending request to Ollama ({task_description})... (Attempt {retries + 1}/{config.MAX_API_RETRIES})[/yellow]", border_style="dim")
                )
                
                if allow_stream:
                    # Streaming response
                    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), transient=True) as progress:
                        task = progress.add_task(description="[cyan]Ollama is thinking...", total=None)
                        
                        self.console.print(f"[cyan]>>> Ollama Response ({task_description}):[/cyan]")
                ...
                self.console.print(Panel("[green]✓ Ollama response received successfully.[/green]", border_style="dim"))
                ...
                if "connection" in str(e).lower():
                    self.console.print("[red]Cannot connect to Ollama server. Make sure Ollama is running.[/red]")
                    ...
                    self.console.print(f"[yellow]Run: ollama pull {config.OLLAMA_MODEL_NAME}[/yellow]")
                    return None
